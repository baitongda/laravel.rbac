...
class Access extends Admin
{
    ...
    protected $fillable = ['role_id', 'node_id','pid','level','module'];//开启白名单字段

    // 获取所有信息
    public function getAllAccess($where = [] ,$field = ['*'] ,  $limit=1000) {
        return $this->where($where)->limit($limit)->select($field)->get();
    }
