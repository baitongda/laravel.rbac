...
class RBAC {
    ...
    static public function authenticate($map,$model='') {
        if(empty($model)) $model =  config('admin.USER_AUTH_MODEL');
        //使用给定的Map进行认证
        return DB::table($model)->where($map)->first();
    }

    //用于检测用户权限的方法,并保存到Session中
    static function saveAccessList($authId=null) {
        if(null===$authId)   $authId = session(config('admin.USER_AUTH_KEY'));
        // 如果使用普通权限模式，保存当前用户的访问权限列表
        // 对管理员开发所有权限
        if(config('admin.USER_AUTH_TYPE') !=2 && !session(config('admin.ADMIN_AUTH_KEY')) )
    ...
    static function checkAccess() {
        ...
        if( config('admin.USER_AUTH_ON') ){
			$_module	=	array();
			$_action	=	array();
            if("" != config('admin.REQUIRE_AUTH_MODULE')) {
                //需要认证的模块
                $_module['yes'] = explode(',',strtoupper(config('admin.REQUIRE_AUTH_MODULE')));
            }else {
                //无需认证的模块
                $_module['no'] = explode(',',strtoupper(config('admin.NOT_AUTH_MODULE')));
            }
            //检查当前模块是否需要认证
            if((!empty($_module['no']) && !in_array(strtoupper(getCurrentControllerName()),$_module['no'])) || (!empty($_module['yes']) && in_array(getCurrentControllerName(),$_module['yes']))) {
				if("" != config('admin.REQUIRE_AUTH_ACTION')) {
					//需要认证的操作
					$_action['yes'] = explode(',',strtoupper(config('admin.REQUIRE_AUTH_ACTION')));
				}else {
					//无需认证的操作
					$_action['no'] = explode(',',strtoupper(config('admin.NOT_AUTH_ACTION')));
				}
				//检查当前操作是否需要认证
				if((!empty($_action['no']) && !in_array(getCurrentMethodName(),$_action['no'])) || (!empty($_action['yes']) && in_array(strtoupper(getCurrentMethodName()),$_action['yes']))) {
					return true;
				}else {
					return false;
				}
            ...
            if(!session(config('admin.USER_AUTH_KEY'))) {
                if(config('admin.GUEST_AUTH_ON')) {
                    ...
                    if(!Session::has('_ACCESS_LIST'))
                        // 保存游客权限
                        RBAC::saveAccessList(config('admin.GUEST_AUTH_ID'));
                }else{
                    // 禁止游客访问跳转到认证网关
                    redirect(PHP_FILE.config('admin.USER_AUTH_GATEWAY'));
                }
            }
        }
        return true;
    ...
    //权限认证的过滤器方法
    static public function AccessDecision($appName=false) {
        if(!$appName) $appName = "cms";
        //检查是否需要认证
        if(RBAC::checkAccess()) {
            //存在认证识别号，则进行进一步的访问决策
            $accessGuid   =   md5($appName.getCurrentControllerName().getCurrentMethodName());
            if(!session(config('admin.ADMIN_AUTH_KEY'))) {
                if(config('admin.USER_AUTH_TYPE')==2) {
                    //加强验证和即时验证模式 更加安全 后台权限修改可以即时生效
                    //通过数据库进行访问检查
                    $accessList = RBAC::getAccessList(session(config('admin.USER_AUTH_KEY')));
                }else {
                ...
                $module = defined('P_MODULE_NAME')?  P_MODULE_NAME   :   getCurrentControllerName();
                if(!isset($accessList[strtoupper($appName)][strtoupper($module)][strtoupper(getCurrentMethodName())])) {
    ...
    static public function getAccessList($authId) {
        ...
        // Db方式权限数据
        $table = array('role'=>config('admin.RBAC_ROLE_TABLE'),'user'=>config('admin.RBAC_USER_TABLE'),'access'=>config('admin.RBAC_ACCESS_TABLE'),'node'=>config('admin.RBAC_NODE_TABLE'));
	...
	static public function getModuleAccessList($authId,$module) {
        // Db方式
        $table = array('role'=>config('admin.RBAC_ROLE_TABLE'),'user'=>config('admin.RBAC_USER_TABLE'),'access'=>config('admin.RBAC_ACCESS_TABLE'));
